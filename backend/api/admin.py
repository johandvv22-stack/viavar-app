...
class MaquinaAdmin(admin.ModelAdmin):
    list_display = ('codigo', 'nombre', 'ubicacion', 'estado', 'fecha_instalacion', 'capacidad_total', 'api_key')  # Añadido api_key
    list_filter = ('estado', 'fecha_instalacion')
    search_fields = ('codigo', 'nombre', 'ubicacion')
    readonly_fields = ('porcentaje_surtido', 'api_key')  # api_key de solo lectura
...
# ===== NUEVOS MODELOS ESP32 =====

# ===== NUEVOS MODELOS ESP32 =====

@admin.register(Esp32Estado)
class Esp32EstadoAdmin(admin.ModelAdmin):
    list_display = (
        'maquina', 
        'estado', 
        'ultima_conexion', 
    ...
    readonly_fields = ('ultima_conexion', 'memoria_ocupada', 'batch_pendientes')  # ELIMINADO 'ultima_alerta'
    ...
    fieldsets = (
        ...
        ('Estado Actual', {
            'fields': ('estado', 'ultima_conexion')  # ELIMINADO 'ultima_alerta'
        }),
        ('Configuración', {
            'fields': ('intervalo_actual', 'firmware_version')
        }),
        ('Métricas', {
            'fields': ('memoria_ocupada', 'batch_pendientes'),
            'classes': ('collapse',)
        }),
    )
...
class RecargaAdmin(admin.ModelAdmin):
    list_display = ('fecha', 'maquina', 'producto', 'cantidad', 'origen', 'visita')
    list_filter = ('maquina', 'origen', 'fecha')
    search_fields = ('maquina__codigo', 'producto__nombre')
    date_hierarchy = 'fecha'
    
    fieldsets = (
        ('Información básica', {
            'fields': ('maquina', 'producto', 'cantidad')
        }),
        ('Detalles', {
            'fields': ('fecha', 'origen', 'visita', 'inventario')
        }),
    )
    
    def get_readonly_fields(self, request, obj=None):
        if obj:  # Si es una edición
            return ['fecha', 'maquina', 'producto']  # No permitir cambiar estos campos
        return []